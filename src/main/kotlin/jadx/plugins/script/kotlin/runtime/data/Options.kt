...
class JadxScriptOptions(
	...
	fun registerString(
		...
		defaultValue: String = "",
	): ScriptOption<String> = register(name, desc, values, defaultValue) { value ->
		if (value == null) {
			defaultValue
		} else {
	...
	fun registerInt(name: String, desc: String = "", defaultValue: Int = 0): ScriptOption<Int> = register(name, desc, emptyList(), defaultValue.toString(), OptionType.NUMBER) { value ->
		when (value) {
			null -> defaultValue
			else -> value.toInt()
		}
