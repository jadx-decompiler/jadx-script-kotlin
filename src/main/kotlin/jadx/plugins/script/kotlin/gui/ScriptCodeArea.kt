...
class ScriptCodeArea(
	...
	init {
		syntaxEditingStyle = scriptNode.syntaxName
	...
	private fun addAutoComplete(settings: JadxSettings): AutoCompletion {
		val provider = ScriptCompleteProvider(this, scriptData)
	...
	override fun getCodeInfo(): ICodeInfo = node.codeInfo

	override fun getLoadTask(): IBackgroundTask = LoadTask(
		{ node.codeInfo.codeStr },
		{ code ->
			text = code
			setCaretPosition(0)
			setLoaded()
