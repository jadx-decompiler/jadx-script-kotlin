...
import jadx.plugins.script.kotlin.eval.ScriptCompletionResult
import jadx.plugins.script.kotlin.eval.ScriptServices
import jadx.plugins.script.kotlin.eval.ScriptServices.Companion.AUTO_COMPLETE_INSERT_STR
...
class ScriptCompleteProvider(
	...
	private val completions: List<Completion>
		get() {
			try {
				...
				val scriptServices = ScriptServices(scriptData)
	...
	override fun getCompletionsAt(comp: JTextComponent, p: Point): List<Completion> = this.completions

	override fun getCompletionsImpl(comp: JTextComponent): List<Completion> = this.completions

	override fun getParameterizedCompletions(tc: JTextComponent): List<ParameterizedCompletion>? = null
}
